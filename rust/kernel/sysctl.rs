...
impl<T: SysctlStorage> Sysctl<T> {
    ...
    pub fn register(
        ...
        let mut table:Vec<bindings::ctl_table> = Vec::try_with_capacity(2)?;
        ...
        let result = unsafe { bindings::register_sysctl_sz(path.as_char_ptr(), table.as_mut_ptr(), 1) };
